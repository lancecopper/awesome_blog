...
#1如果url处理函数需要传入关键字参数，且默认是空得话，获取这个key
...
def has_request_arg(fn):
    ...
    for name, param in params.items():
        if name == 'request':
            found = True
            continue
        #request参数类型不是1[*args],2[*,key],3[**kw]
...
class RequestHandler(object):
    ...
    def __call__(self, request):
        kw = None
        logging.info(' %s : has_request_arg = %s,  has_var_kw_arg = %s, has_named_kw_args = %s, get_named_kw_args = %s, get_required_kw_args = %s ' % (__name__, self._has_request_arg, self._has_var_kw_arg, self._has_named_kw_args,self._named_kw_args ,self._required_kw_args))
        #如果处理函数1[url处理函数的参数是**kw]或2[需要传入关键字参数]或3[需要传入关键字参数，且默认是空]
